...
This program aims to recreate a simplified version of the classic
Windows game 'Minesweeper'. The aim of the game is to press all the safe
tiles on the board while avoiding all the mine tiles which are randomly
placed alongside safe tiles in an 'm by n' grid; the minefield.
...
streamlined to reset the current game with a click of the reset button.
...
minefield is pre-rendered and thus the player runs the risk of pressing
a mine tile as their first tile, thus ending the game with a score of 0.
...
class Minesweeper:
    ...
    def create_minefield(self, difficulty):
        ...
        # minefields overlapping and causing, to put it in techical
        # terms, a mess.
        try:
